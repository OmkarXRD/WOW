package com.live.worldsocialintegrationapp.Fragments.Home.Related.RelatedLive;
...
import com.live.worldsocialintegrationapp.Adapters.FollowingLiveUsersRVAapter;
...
public class FollowingLiveUsersFragment extends Fragment implements FollowingLiveUsersRVAapter.Callback {

    private RecyclerView followingLiveRV;
    FollowingLiveUsersRVAapter followingLiveUsersRVAapter;
    List<GetFollowingLiveRoot.Detail> list;
    private TextView noLiveFollowingTv;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_following_live_users, container, false);
    }
    ...
    private void getFollowingLiveUsers() {
        ...
        if(id != null){
            new Mvvm().getFollowingLiveUsers(requireActivity(),id, AppConstants.USER_ID).observe(requireActivity(), new Observer<GetFollowingLiveRoot>() {
                @Override
                public void onChanged(GetFollowingLiveRoot getFollowingLiveRoot) {

                    if(getFollowingLiveRoot != null) {
                        if (getFollowingLiveRoot.getSuccess().equalsIgnoreCase("1")) {

//                        Toast.makeText(requireContext(), "1 "+getFollowingLiveRoot.getMessage(), Toast.LENGTH_SHORT).show();
                            list = new ArrayList<>();
                            list = getFollowingLiveRoot.getDetails();

                            if (list.isEmpty()) {
                                noLiveFollowingTv.setVisibility(View.VISIBLE);
                            } else {
                                noLiveFollowingTv.setVisibility(View.GONE);
                                followingLiveUsersRVAapter = new FollowingLiveUsersRVAapter(list, requireContext(), FollowingLiveUsersFragment.this);
                                followingLiveRV.setAdapter(followingLiveUsersRVAapter);
                            }
                        } else {
                            if (getContext() != null) {
                                noLiveFollowingTv.setVisibility(View.VISIBLE);
//                            Toast.makeText(requireContext(), "0 "+getFollowingLiveRoot.getMessage(), Toast.LENGTH_SHORT).show();
                            }
...
//                Toast.makeText(requireContext(), "id is null", Toast.LENGTH_SHORT).show();
    ...
    private void findIds(View view) {
        followingLiveRV=view.findViewById(R.id.followingLiveRV);
        noLiveFollowingTv=view.findViewById(R.id.noLiveFollowingTv);
    }


    @Override
    public void callback(GetFollowingLiveRoot.Detail detail,int pos) {
        startLive(detail,pos);
    }

    @Override
    public void enterPassword(GetFollowingLiveRoot.Detail detail, int pos) {
        ...
        confirm_pin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (liveLock.getOTP().equalsIgnoreCase(detail.getLivePassword())) {
                    startLive(detail, pos);
    ...
    private void startLive(GetFollowingLiveRoot.Detail detail,int pos){
