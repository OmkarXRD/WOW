...
import com.live.worldsocialintegrationapp.Adapters.FriendsLiveUserRVAdapter;
...
public class FriendLiveUsersFragment extends Fragment implements FriendsLiveUserRVAdapter.Callback {

    private RecyclerView friendLiveUsersRV;
    private List<GetFollowingLiveRoot.Detail> list;
    private FriendsLiveUserRVAdapter friendsLiveUserRVAdapter;
    private TextView noLiveFriendTv;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_friend_live_users, container, false);
    }
...
//        friendLiveUsersRV.setAdapter(new FollowingLiveUsersRVAapter());

        getFriendLive();
    }

    private void getFriendLive() {
        String id = App.getSharedpref().getString("userId");
        if(id != null) {
            new Mvvm().getFriendsLive(requireActivity(), id).observe(requireActivity(), new Observer<GetFollowingLiveRoot>() {
                @Override
...
//                            Toast.makeText(requireContext(), "1 " + getFollowingLiveRoot.getMessage(), Toast.LENGTH_SHORT).show();
                            ...
                            if(list.isEmpty()){
                                noLiveFriendTv.setVisibility(View.VISIBLE);
                            }else{
                                noLiveFriendTv.setVisibility(View.GONE);
                                try {
                                    friendsLiveUserRVAdapter = new FriendsLiveUserRVAdapter(list, requireContext(), FriendLiveUsersFragment.this);
                                    friendLiveUsersRV.setAdapter(friendsLiveUserRVAdapter);
                                }catch (Exception e){
...
//            Toast.makeText(requireContext(), "id null", Toast.LENGTH_SHORT).show();
    ...
    private void findIds(View view) {
        friendLiveUsersRV=view.findViewById(R.id.friendLiveUsersRV);
        noLiveFriendTv=view.findViewById(R.id.noLiveFriendTv);
    }
